...
\paragraph{} The core challenge, which we call the \textbf{Ultimate Team Optimization Problem}, arises from the conflict between these two objectives. Often, the highest-rated players do not share the same club or league, making it difficult to achieve high chemistry. Conversely, building a team with perfect chemistry might require using lower-rated players, thus sacrificing overall team talent. More specifically, the collection of players available is limited by the ressources (in-game currency, player items) of the user. Thus, this problem is even more constrained as the user has its ressources limited.
...
Standard MILP solvers, such as Google OR-Tools, are designed to handle single-objective problems. Therefore, we must employ a strategy to explore the solution space and construct the Pareto frontier ourselves. The chosen approach is an iterative search with dominance-breaking constraints.

\subsection{Scalarized Objective Function}
To utilize standard MILP solvers, we convert the multi-objective problem into a single-objective one by scalarizing the objectives. We use a simple weighted sum with a fixed weight $\alpha \in [0, 1]$ (typically 0.5):
\begin{equation}
\text{Maximize} \quad (1-\alpha) \left( \frac{Z_{Rating}}{Z_{Rating}^{max}} \right) + \alpha \left( \frac{Z_{Chemistry}}{Z_{Chemistry}^{max}} \right)
\end{equation}
Where $Z^{max}$ terms are normalization constants. While traditionally one might vary $\alpha$ to trace the Pareto frontier, our approach relies on a more robust iterative search mechanism.

\subsection{Iterative Search and Pareto Constraints}
The core of our methodology is an iterative search process that guarantees the exploration of the Pareto frontier. Since the solver is constrained to never return a solution that is dominated by or identical to a previously found solution, it is forced to find a new point on the Pareto front in each iteration. This renders the variation of the weight $\alpha$ largely redundant; even with a fixed weight, the constraints drive the solver to discover the entire frontier.

Let $\mathcal{P}$ be the set of all Pareto-optimal teams found so far. In each new iteration, we add constraints to ensure the next solution found is not dominated by any solution $p \in \mathcal{P}$.
...
\end{gather}
This is the main logic constraint. It forces the solver into one of three situations:
\begin{enumerate}
    \item The new solution is strictly better in rating ($\delta^{rating}_p=1$).
    \item The new solution is strictly better in chemistry ($\delta^{chem}_p=1$).
    \item The new solution has identical objective values to team $p$. In this case, both $\delta$ variables must be 0. The constraint can then only be satisfied if both $\rho$ variables are forced to be 1.
\end{enumerate}
The next constraints define when the $\delta$ and $\rho$ variables can be activated. The subtlety of the model lies in how the $\rho$ variables allow for the third case.
\begin{gather}
...
\end{gather}
These constraints define the ratio variables. Consider the rating ratio $\rho^{rating}_p$. If the new solution's rating is greater than $R_p$, then $\rho^{rating}_p$ can be greater than 1. If the new rating is less than $R_p$, $\rho^{rating}_p$ will be less than 1. Crucially, if the new rating is exactly equal to $R_p$, the constraint becomes $R_p \cdot \rho^{rating}_p \le R_p$, which implies $\rho^{rating}_p \le 1$. Since the main logic constraint (25) is trying to maximize its left-hand side, the solver will push $\rho^{rating}_p$ to its maximum possible value, which is exactly 1 in this case. This is the mechanism that allows the model to find alternative teams with the same objective scores, satisfying constraint (25) via the $\rho$ variables when the $\delta$ variables are both zero.
...
\begin{algorithmic}[1]
\STATE \textbf{Input:} Player set $I$, list of Formations $F$
...
\FOR{each formation $f$ in $F$}
    ...
    \WHILE{keep\_searching}
        \STATE Build MILP model for formation $f$
        ...
        \ELSE
            \STATE keep\_searching $\leftarrow$ false \COMMENT{No more solutions for this formation}
        \ENDIF
    \ENDWHILE
\ENDFOR
\RETURN $\mathcal{P}$
